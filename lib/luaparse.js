...
(function (root, name, factory) {
	...
	let EOF = 1;
	let StringLiteral = 2
	let Keyword = 4, Identifier = 8
	let NumericLiteral = 16
	let Punctuator = 32
	let BooleanLiteral = 64
	let NilLiteral = 128
	let VarargLiteral = 256;
	...
	let ast = exports.ast = {
		...
		binaryExpression: function(operator, left, right) {
			let type = ('and' === operator || 'or' === operator) ?
				'LogicalExpression' :
				'BinaryExpression';
	...
	let indexOf = function indexOf(array, element) {
	...
	function lex() {
		...
		let next = input.charCodeAt(index + 1);
	...
	function consumeEOL() {
		let charCode = input.charCodeAt(index)
		let peekCharCode = input.charCodeAt(index + 1);
	...
	function readHexLiteral() {
		...
		let digit, fractionStart, exponentStart, digitStart;
	...
	function readEscapeSequence() {
		...
		switch (input.charAt(index)) {
			...
			case '5': case '6': case '7': case '8': case '9':
				...
				let ddd = parseInt(input.slice(sequenceStart, index), 10);
				if (ddd > 255) {
	...
	function scanComment() {
		...
		let lineStartComment = lineStart;
		let lineComment = line;
		...
		if (options.comments) {
			let node = ast.comment(content, input.slice(tokenStart, index));
	...
	function createScope() {
		let scope = Array.apply(null, scopes[scopeDepth++]);
		scopes.push(scope);
	...
	function parseForStatement() {
		...
		if (consume('=')) {
			...
			let step = consume(',') ? parseExpectedExpression() : null;
	...
	function parseFunctionDeclaration(name, isLocal) {
		...
		let parameterIndex = (name && name.indexer === ':') ? 1 : 0
	...
	function parseExpression() {
		let expression = parseSubExpression(0);
		return expression;
	}
		...
		if (Punctuator === token.type) {
			switch (token.value) {
				...
				case '{':
					markLocation();
					next();
					let table = parseTableConstructor();
	...
	function parsePrimaryExpression() {
		let literals = StringLiteral | NumericLiteral | BooleanLiteral | NilLiteral | VarargLiteral;
		...
		if (type & literals) {
			pushLocation(marker);
			let raw = input.slice(token.range[0], token.range[1]);
			next();
